...
- [ ] **TOOL-01**: A shared scenario description (SSOT) defines positions, antenna
...
- [ ] **TOOL-02**: The tool owns an explicit, versioned coordinate/units transform
  (origin, axes/handedness, scale, units) between OMNeT++ coordinates and the Sionna scene.
- [ ] **TOOL-03**: The tool runs one batched `PathSolver` over all Tx/Rx pairs and extracts
...
- [ ] **ART-01**: The tool emits an HDF5 artifact + JSON manifest sidecar carrying a
  `schema_version` field checked at load time.
- [ ] **ART-02**: The manifest carries the full parameter contract, the `coord_transform`
...
- [ ] **CAL-01**: A reference / empty-world (obstacle-free) calibration mode validates the
