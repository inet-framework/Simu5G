...
class Binder : public cSimpleModule
{
  private:
    ...
    int16_t multicastDestIdCounter_;
  ...
  public:
    Binder() :  lastUpdateUplinkTransmissionInfo_(0.0), lastUplinkTransmission_(0.0)
    {
        ...
        multicastDestIdCounter_ = num(MULTICAST_DEST_MIN_ID);
    }
     ...
     */
    void registerServingNodeB(MacNodeId enbId, MacNodeId ueId);
    ...
    void joinMulticastGroup(MacNodeId nodeId, MacNodeId multicastDestId);
    // check if the node is enrolled in the group
    bool isInMulticastGroup(MacNodeId nodeId, MacNodeId multicastDestId);
  ...
  private:
    void createIncomingConnectionOnNode(MacNodeId nodeId, FlowControlInfo *lteInfo);
    void createOutgoingConnectionOnNode(MacNodeId nodeId, FlowControlInfo *lteInfo);
