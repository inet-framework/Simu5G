...
class ID2dChannelModel
{
  public:
    ...
    virtual std::vector<double> getSINR_D2D(UserControlInfo *lteInfo, MacNodeId peerUeId, inet::Coord peerUeCoord, MacNodeId enbId = NODEID_NONE) = 0;

    /// Compute the D2D SINR from a precomputed RSRP vector (used for one-to-many D2D reception).
    virtual std::vector<double> getSINR_D2D(UserControlInfo *lteInfo_1, MacNodeId destId, inet::Coord destCoord, MacNodeId enbId, const std::vector<double>& rsrpVector) = 0;
};
