...
{
    // The L2 ID a pseudo node id stands for, whether it names a peer UE or a
    // group: callers key bearers and address SCIs by the pseudo id and need the
    // real 24-bit identity back, without caring which kind it is.
    auto it = nodeIdToL2Id_.find(nodeId);
    if (it != nodeIdToL2Id_.end())
        return it->second;
    auto git = groupPidToL2Id_.find(nodeId);
    return git != groupPidToL2Id_.end() ? git->second : SL_L2ID_NONE;
}
...
void SlBinder::establishSlConnection(const FlowId& flow, const BearerRequest& req)
{
    ...
    senderRrc->second->createSlOutgoingConnection(flow, req);

    if (req.slCastType == SL_UNICAST)
    ...
    for (MacNodeId member : getGroupMembers(getL2IdForNodeId(flow.destId))) {
        if (member == senderId)
            continue;
        ...
        memberRrc->second->createSlIncomingConnection(flow, req);
    }
}
