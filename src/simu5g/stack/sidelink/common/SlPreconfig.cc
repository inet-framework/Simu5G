...
void SlPreconfig::loadFromJson(const cValueMap *map)
{
    ...
    if (map->containsKey("txPercentage"))
        txPercentage = map->get("txPercentage").doubleValue();
    ...
    if (txPercentage <= 0 || txPercentage > 1)
        throw cRuntimeError("SlPreconfig: txPercentage must be in (0,1], got %g", txPercentage);
