...
void NrSlMacUe::handleTxSlot()
{
    ...
    for (auto& [cid, connInfo] : connDescOut_) {
        if (connInfo.buffer->isEmpty())
            continue;
        int prio = cfg.getPqiPriority(getLogicalChannelConfig(cid).slPqi);
    ...
    if (bestDest != NODEID_NONE) {
        ...
        for (auto& [cid, connInfo] : connDescOut_)
            if (cid.getNodeId() == bestDest && !connInfo.buffer->isEmpty())
                order.emplace_back(cfg.getPqiPriority(getLogicalChannelConfig(cid).slPqi), cid);
        ...
        for (const auto& [prio, cid] : order) {
            ...
            const LogicalChannelConfig& lc = getLogicalChannelConfig(cid);
            unsigned int snBits = lc.snFieldLength;
            bool isAm = (lc.rlcType == AM);
...
void NrSlMacUe::macPduMake(MacCid cid)
{
    ...
    for (auto& [destId, cids] : byDest) {
        ...
        // Per-destination fields, shared across that destination's bearers. The
        // 24-bit L2 IDs are the ones the sending and receiving pseudo node ids
        // stand for; the cast type comes from the bearer's channel configuration.
        const LogicalChannelConfig& destLc = getLogicalChannelConfig(cids.front());
        SlL2Id dstL2Id = slBinder_->getL2IdForNodeId(destId);
        auto slInfo = macPkt->addTagIfAbsent<SlTxInfoTag>();
        slInfo->setSrcL2Id(slRrc_->getSrcL2Id());
        slInfo->setDstL2Id(dstL2Id);
        slInfo->setCastType(destLc.slCastType);
            ...
            switch (destLc.slCastType) {
                ...
                case SL_GROUPCAST: {
                    ...
                    if (slrb != nullptr && slrb->psfchMode == SL_PSFCH_NACK_ONLY) {
                        ...
                        feedback = true;
                        expectedAcks = (int)slBinder_->getGroupMembers(dstL2Id).size() - 1;
                    }
                    break;
                }
                default:
...
int NrSlMacUe::mode1BsrBytes() const
{
    ...
    for (const auto& [cid, connInfo] : connDescOut_) {
        ...
        if (backlog <= 0)
            continue;
        size += (int)backlog;
        size += (getLogicalChannelConfig(cid).rlcType == AM) ? RLC_HEADER_AM : RLC_HEADER_UM;
    }
    return size;
}
