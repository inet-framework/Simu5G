...
class SlIp2Nic : public Ip2Nic
{
  protected:
    ...
    /// Addressing of a sidelink bearer toward a destination pseudo node id.
    FlowId makeSlFlowId(MacNodeId dstPid, DrbId drbId) const;
    /// The PC5 half of a bearer establishment request, from its configuration entry.
    static BearerRequest makeSlBearerRequest(const SlrbConfigEntry& slrb);
