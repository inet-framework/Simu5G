...
void SlRrc::checkSlrbRlcMode(const FlowControlInfo *lteInfo)
{
    // TR 38.885 5.4.2: a UM RLC entity serves broadcast, groupcast and unicast;
    // RLC AM is supported for sidelink unicast only. TM serves the SL-SRB
    // carrying PC5-RRC, which is a unicast link bearer as well.
    SlCastType cast = (SlCastType)lteInfo->getSlCastType();
    LteRlcType rlc = (LteRlcType)lteInfo->getRlcType();
    if ((rlc == AM || rlc == TM) && cast != SL_UNICAST)
        throw cRuntimeError("SlRrc: RLC %s is only supported on sidelink unicast links "
                            "(TR 38.885 5.4.2), but SLRB %d toward L2 ID 0x%06x is %s",
                            rlc == AM ? "AM" : "TM", (int)num(lteInfo->getDrbId()),
                            lteInfo->getSlDstL2Id(), slCastTypeToA(cast).c_str());
}

void SlRrc::createSlOutgoingConnection(FlowControlInfo *lteInfo)
{
    Enter_Method_Silent("createSlOutgoingConnection()");
    checkSlrbRlcMode(lteInfo);
