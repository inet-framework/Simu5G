...
simsignal_t NrAmTxQueue::receivedPacketFromUpperLayerSignal_ = registerSignal("receivedPacketFromUpperLayer");
simsignal_t NrAmTxQueue::sentPacketToLowerLayerSignal_ = registerSignal("sentPacketToLowerLayer");
...
void NrAmTxQueue::initialize(int stage)
{
...
{
    // Notify the MAC that there is pending data so it schedules a grant. The MAC's
    // virtual queue de-duplicates, so an unconditional notification is safe.
    unsigned int pendingData = getPendingDataVolume();
    if (pendingData > 0 && lteInfo_) {
...
void NrAmTxQueue::processControlPacket(Packet *pktPdu)
{
    ...
    for (size_t i = 0; i < data.nacks.size(); ++i) {
        ...
        for (unsigned int j = 0; j < info.nackRange; ++j) {
            ...
            if (txBuffer_->isInRtxRange(nackedSn)) {
                ...
                if (!added) {
                    // Max retransmissions reached — Radio Link Failure. RLF teardown is
                    // not part of this port; just stop transmitting on this entity.
...
void NrAmTxQueue::bufferControlPduInternal(inet::Packet *pkt)
{
    controlBuffer_.push_back(pkt);
    sendNewDataNotification(pkt);
}

void NrAmTxQueue::sendPduToMac(inet::Packet *pkt)
{
...
void NrAmTxQueue::sendNewDataNotification(inet::Packet *pkt)
{
    auto newData = new inet::Packet("AM-NewData");
    newData->copyTags(*pkt);
    // The MAC virtual queue reads the original packet length from PdcpTrackingTag.
    // SDU notifications already carry it; for status/buffer notifications built from
    // fresh packets, supply the announced data volume here.
...
void NrAmTxQueue::handleMessage(cMessage *msg)
{
    ...
    if (incoming->isName("in")) {
        // SDU from the upper mux (PDCP)
        auto pkt = check_and_cast<Packet *>(msg);
        emit(receivedPacketFromUpperLayerSignal_, pkt);

        // Add PDCP tracking information (used for SDU sequence/length bookkeeping)
        ...
        throw cRuntimeError("NrAmTxQueue: unexpected message from gate %s", incoming->getFullName());
    }
}
