...
#ifndef STACK_PHY_CHANNELMODEL_RADIO_H_
#define STACK_PHY_CHANNELMODEL_RADIO_H_
 ...
 * environment -- which is what sets this model apart from IdealRadio, its
 * impairment-free sibling. It is far more than propagation, and covers:
 ...
 * medium alongside them -- D2dRadio does not override any of
 ...
 * subclasses).
 *
 * D2D links are evaluated by the medium too; D2dRadio is an
 * endpoint marker (RadioMedium::RadioDescriptor::d2dEndpoint) plus the
 * ID2dRadio entry points (getRSRP_D2D/getSINR_D2D) D2D-aware PHY code
 * calls directly.
 */
class Radio : public RadioBase
{
  ...
  protected:
     ...
     * D2dRadio::getRSRP_D2D() (a subclass, reached through an
     ...
     * getRSRP(const RadioLink&, double) above: only D2dRadio's own
     * one-to-many capture-effect path (D2dRadio::getSINR_D2D())
     * calls it.
     */
...
#endif /* STACK_PHY_CHANNELMODEL_RADIO_H_ */
