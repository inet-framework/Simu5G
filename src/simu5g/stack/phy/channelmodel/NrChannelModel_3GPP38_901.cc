...
double NrChannelModel_3GPP38_901::computePenetrationLoss(double threeDimDistance)
{
    ...
    double pLoss_tw = 0.0;
    if (carrierFrequency_ <= GHz(6.0))
        pLoss_tw = 20.0;
    else {
        double Lglass = 2 + 0.2 * carrierFrequency_.get();
        double LiirGlass = 23 + 0.3 * carrierFrequency_.get();
        double Lconcrete = 5 + 4 * carrierFrequency_.get();
...
double NrChannelModel_3GPP38_901::computeUrbanMacro(double threeDimDistance, double twoDimDistance, bool los)
{
    ...
    double dbp = 4 * hNodeB * hUe * (Hz(carrierFrequency_).get()  / SPEED_OF_LIGHT);

    // Compute LOS path loss
    double pLoss_los = 0.0;
    if (twoDimDistance < dbp)
        pLoss_los = 28 + 22 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_;
    else
        pLoss_los = 28 + 40 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_ - 9 * log10((dbp * dbp + (hNodeB_ - hUe_) * (hNodeB_ - hUe_)));
    ...
    if (los)
        ...
        double pLoss_nlos = 13.54 + 39.08 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_ - 0.6 * (hUe_ - 1.5);
...
double NrChannelModel_3GPP38_901::computeUrbanMicro(double threeDimDistance, double twoDimDistance, bool los)
{
    ...
    double pLoss_los = 0.0;
    if (twoDimDistance < dbp)
        pLoss_los = 32.4 + 21 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_;
    else
        pLoss_los = 32.4 + 40 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_ - 9.5 * log10((dbp * dbp + (hNodeB_ - hUe_) * (hNodeB_ - hUe_)));
    pLoss_los += penetrationLoss;
    ...
    if (los)
        ...
        // Compute NLOS path loss
        double pLoss_nlos = 35.3 * log10(threeDimDistance) + 22.4 + 21.3 * log10_carrierFrequencyGHz_ - 0.3 * (hUe_ - 1.5);
        pLoss_nlos += penetrationLoss;
...
double NrChannelModel_3GPP38_901::computeRuralMacro(double threeDimDistance, double twoDimDistance, bool los)
{
    ...
    // Compute break-point distance
    double dbp = 2 * M_PI * hNodeB_ * hUe_ * (Hz(carrierFrequency_).get()  / SPEED_OF_LIGHT);
    ...
    double pLoss_los = 0.0;
    if (twoDimDistance < dbp) {
        pLoss_los = 20 * log10(40 * M_PI * threeDimDistance * (carrierFrequency_.get() / 3.0)) + min1 * log10(threeDimDistance) - min2 + 0.002 * log10(h) * threeDimDistance;
    }
    else {
        pLoss_los = 20 * log10(40 * M_PI * dbp * (carrierFrequency_.get() / 3.0)) + min1 * log10(dbp) - min2 + 0.002 * log10(h) * dbp
    ...
    if (los)
        ...
        double pLoss_nlos = 161.04 - 7.1 * log10(W) + 7.5 * log10(h) - (24.37 - 3.7 * pow(h / hNodeB_, 2)) * log10(hNodeB_)
            + (43.42 - 3.1 * log10(hNodeB_)) * (log10(threeDimDistance) - 3.0) + 20 * log10_carrierFrequencyGHz_
...
double NrChannelModel_3GPP38_901::computeIndoor(double threeDimDistance, double twoDimDistance, bool los)
{
    ...
    // Compute LOS path loss
    double pLoss_los = 32.4 + 17.3 * log10(threeDimDistance) + 20 * log10_carrierFrequencyGHz_;
    pLoss_los += penetrationLoss;
    ...
    if (los)
        ...
        // Compute NLOS path loss
        double pLoss_nlos = 38.3 * log10(threeDimDistance) + 17.3 + 24.9 * log10_carrierFrequencyGHz_;
        pLoss_nlos += penetrationLoss;
