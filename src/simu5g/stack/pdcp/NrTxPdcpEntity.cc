...
void NrTxPdcpEntity::deliverPdcpPdu(Packet *pkt)
{
    ...
    if (getNodeTypeById(pdcp_->nodeId_) == UE) {
        EV << NOW << " NrTxPdcpEntity::deliverPdcpPdu - LCID[" << lteInfo->getDrbId() << "] - sending packet to lower layer" << endl;
        ...
        if (!pdcp_->isDualConnectivityEnabled()) {
            EV << NOW << " NrTxPdcpEntity::deliverPdcpPdu - LCID[" << lteInfo->getDrbId() << "] - the destination is a UE. Sending packet to lower layer" << endl;
            LteTxPdcpEntity::deliverPdcpPdu(pkt);
        }
        else {
            ...
            if (!useNR) {
                EV << NOW << " NrTxPdcpEntity::deliverPdcpPdu - LCID[" << lteInfo->getDrbId() << "] useNR[" << useNR << "] - the destination is a UE. Sending packet to lower layer." << endl;
                LteTxPdcpEntity::deliverPdcpPdu(pkt);
            }
            else { // useNR
                EV << NOW << " NrTxPdcpEntity::deliverPdcpPdu - LCID[" << lteInfo->getDrbId() << "] - the destination is under the control of a secondary node" << endl;
