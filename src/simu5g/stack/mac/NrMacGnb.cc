...
// NOTE: this method is a clone of LteMacEnbD2D::macPduUnmake (minus nothing --
// that version references no D2D state): it preserves the pre-separation NR gNB
// behavior, which followed the LteMacEnbD2D fork rather than LteMacEnb. The
// load-bearing difference from LteMacEnb::macPduUnmake is the BSR buffer key:
// MacCid(sourceId, lteInfo->getPacketLcid()) instead of MacCid(sourceId,
// LogicalCid(0)) -- and the matching ownership semantics (bufferizeBsr keeps
// the CE; no delete here). Defect logged; forks to be reconciled later.
void NrMacGnb::macPduUnmake(cPacket *cpkt)
{
    ...
    while (macPdu->hasSdu()) {
        ...
        *upPkt->addTag<FlowControlInfo>() = connDescIn_[cid].toFlowControlInfo();
        ...
        EV << "NrMacGnb: Lcid --->"<< (int)lcid << " Cid: " << cid <<endl;
    ...
    while (macPdu->hasCe()) {
        ...
        LogicalCid lcid = lteInfo->getPacketLcid();  // for a non-D2D UE this is always SHORT_BSR
...
// NOTE: this method is a clone of LteMacEnbD2D::sendGrants (minus nothing --
// that version references no D2D state): it preserves the pre-separation NR gNB
// behavior, which followed the LteMacEnbD2D fork rather than LteMacEnb. The
// load-bearing differences from LteMacEnb::sendGrants are (1) the grant chunk
// length set to b(1) (1 bit) instead of the 1-byte message default, and (2) the
// grant direction taken from the BSR LCID via directionFromBsrLcid() -- a pure
// LCID mapping that returns the UL fallback for the SHORT_BSR presented by
// non-D2D UEs. Defect logged; forks to be reconciled later.
void NrMacGnb::sendGrants(std::map<GHz, LteMacScheduleList> *scheduleList)
{
    ...
    for (auto& [carrierFreq, carrierScheduleList] : *scheduleList) {
        while (!carrierScheduleList.empty()) {
            ...
            EV << NOW << " NrMacGnb::sendGrants Node[" << getMacNodeId() << "] - "
            ...
            for ( ; cw < codewords; ++cw) {
                ...
                EV << NOW << " NrMacGnb::sendGrants - granting " << grantedBytes << " on cw " << cw << endl;
            }
            RbMap map;
