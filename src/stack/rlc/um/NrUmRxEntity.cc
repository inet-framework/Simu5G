...
void NrUmRxEntity::enque(cPacket *pktAux)
{
    ...
    if (pdu->getFramingInfo()==0) {
        ...
        //std::cout<<name_entity<<"; "<< NOW << " NrUmRxEntity::enque - passing complete pdu "<< pktPdu << endl;
        toPdcp(pktSdu);
        pktSdu = nullptr;
...
void NrUmRxEntity::handlePDUInReceivedBuffer(inet::Ptr<NrRlcUmDataPdu> pdu,  unsigned int tsn) {
    ...
    if (complete) { //If not complete either it has been discarded or it is buffered
        ...
        //std::cout<<name_entity<<"; "<< NOW << " NrUmRxEntity::handlePDUInReceivedBuffer()  completed remaining and passing up sdu - "<< sdu << endl;
        toPdcp(sdu);
        sdu = nullptr;
    }
...
void NrUmRxEntity::toPdcp(Packet *pktAux)
{
    ...
    if (ue != nullptr) {
        if (lteInfo->getDirection() != D2D && lteInfo->getDirection() != D2D_MULTI) { // UE in IM
            ...
            //std::cout<<name_entity<<NOW<<": NrUmRxEntity::toPdcp"<<this<<";length="<<length<<"tpsample="<<tpsample<<";;interval="<<interval<<"; totalRcvdBytes_="<<totalRcvdBytes_<<"; time="<<(NOW - getSimulation()->getWarmupPeriod())<<endl;
            ...
            if (interval>=1) {
                ...
                ////std::cout<<name_entity<<NOW<<": NrUmRxEntity::toPdcp"<<this<<";length="<<length<<"tpsample="<<tpsample<<"; rlcThroughputSample_="<<tput<<";interval="<<interval<<"; totalRcvdBytes_="<<totalRcvdBytes_<<"; time="<<(NOW - getSimulation()->getWarmupPeriod())<<endl;
