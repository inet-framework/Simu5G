...
bool RlcUmReceptionBuffer::handleSegment(uint32_t sn, uint32_t totalLen, uint32_t start, uint32_t end, Packet* ptr) {
    ...
    if (isDiscardable) {
        std::cout << "[RLC UM RX] Discarding SN=" << sn << " (Already Reassembled)" << std::endl;
        //We can delete it here, it is not used anymore
        delete ptr;
        return false;
    }
    // Place in buffer
    ...
    if (isNowComplete) {
        ...
        if (!isWithinWindow(RX_Next_Reassembly)) {
            //eegea: TODO We may be leaking sdu pointers here, maybe we should try to remove the ones that may be in between
