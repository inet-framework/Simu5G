...
public:
     ...
     */
    bool handleSegment(uint32_t sn, uint32_t totalLen, uint32_t start, uint32_t end, Packet* ptr);
    ...
    void clearBuffer() {
        ...
        sduBuffer.clear();
        std::cout << "[RLC UM RX] Buffer cleared and packet pointers deleted." << std::endl;
    }
