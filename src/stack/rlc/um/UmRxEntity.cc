    ...
    if (ue != nullptr) {
        if (lteInfo->getDirection() != D2D && lteInfo->getDirection() != D2D_MULTI) { // UE in IM
            ...
            if (interval>=1) {
                ...
                //std::cout<<NOW<<": UmRxEntity::toPdcp"<<this<<";length="<<length<<"tpsample="<<tpsample<<"; rlcThroughputSample_="<<tput<<";interval="<<interval<<"; totalRcvdBytes_="<<totalRcvdBytes_<<"; time="<<(NOW - getSimulation()->getWarmupPeriod())<<endl;
