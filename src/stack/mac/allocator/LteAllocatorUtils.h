...
struct AllocatedRbsPerBandInfo
{
    ...
    unsigned int allocated_ = 0; // Initialize inline
...
struct AllocatedRbsPerUeInfo
{
    ...
    unsigned int allocatedBlocks_ = 0; // Initialize inline
    /// Stores the amount of bytes allocated to every UE in the structure band
    unsigned int allocatedBytes_ = 0; // Initialize inline

    // if false this user is not using MU-MIMO
    bool muMimoEnabled_ = false; // Initialize inline
    // if false this user transmits on MAIN_PLANE, otherwise it is considered as secondary
    bool secondaryUser_ = false; // Initialize inline
    MacNodeId peerId_ = 0; // Initialize inline
    ...
    Remote currentAntenna_ = MACRO; // Initialize inline
