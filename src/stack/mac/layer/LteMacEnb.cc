...
void LteMacEnb::deleteQueues(MacNodeId nodeId)
{
    ...
    for (bit = bsrbuf_.begin(); bit != bsrbuf_.end(); ) {
        if (MacCidToNodeId(bit->first) == nodeId) {
            ...
            bit = bsrbuf_.erase(bit); // Delete Element
        }
        else {
            ++bit;
        }
...
void LteMacEnb::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL) {
        ...
        for ( ; it != carriers->end(); ++it, ++i) {
            ...
            bgTrafficManager_[carrierFrequency] = check_and_cast<IBackgroundTrafficManager *>(getParentModule()->getSubmodule("bgTrafficGenerator", i)->getSubmodule("manager")); // TODO "^.bgTrafficGenerator[i].manager"
