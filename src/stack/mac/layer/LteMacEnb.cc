...
void LteMacEnb::initialize(int stage)
{
    ...
    else if (stage == inet::INITSTAGE_LINK_LAYER)
    {
        ...
        for ( ; it != carriers->end(); ++it, ++i)
        {
            ...
            bgTrafficManager_[carrierFrequency] = check_and_cast<BackgroundTrafficManager*>(getParentModule()->getSubmodule("bgTrafficGenerator",i)->getSubmodule("manager")); // TODO "^.bgTrafficGenerator[i].manager"
