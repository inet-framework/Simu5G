...
UserInfo::UserInfo(const LocationInfo& location, const inet::Ipv4Address& address, MacCellId accessPointId, const std::string& resourceUrl, int zoneId) :
    timestamp_(), locationInfo_(location), address_(address), accessPointId_(accessPointId), resourceUrl_(resourceUrl)
{
    timestamp_.setSeconds();
}

UserInfo::UserInfo(const inet::Coord& location, const inet::Coord& speed, const inet::Ipv4Address& address, MacCellId accessPointId, const std::string& resourceUrl) :
    timestamp_(), locationInfo_(location, speed), address_(address), accessPointId_(accessPointId), resourceUrl_(resourceUrl)
{
    timestamp_.setSeconds();
}
