...
run_simulation() {
    ...
    simu5g_dbg $SIM_ARGS --sim-time-limit=$SIMTIME_LIMIT \
       -u Cmdenv --cmdenv-express-mode=false --cmdenv-log-simulation=false --cmdenv-redirect-output=true --result-dir=$results_suffix \
       --cmdenv-output-file='${resultdir}/cmdenv-raw.out' --output-scalar-file='${resultdir}/out.sca'
    cd - > /dev/null
}
